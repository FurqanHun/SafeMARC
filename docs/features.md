...
- [x] **GUI Ghosting & Visual Artifact Fixes**: Eliminates "copied-background" rendering artifacts in the progress dialog by deferring background thread startup with `QTimer.singleShot(50ms)`, giving the window manager time to fully map and paint the dialog before heavy processing begins. Periodic `time.sleep(0.01)` calls inside worker loops yield the Python GIL back to the GUI thread, keeping the `QProgressBar` and spinner animations fluid throughout extraction and compilation.
