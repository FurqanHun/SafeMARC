...
- [x] **Zero-Lag Session Caching & Instant UI Filtering**: An advanced memory dictionary cache that saves full biometric and text hits per file/page, ensuring backwards and forwards queue navigation is instantaneous (<10ms latency). The cache is deeply decoupled into `_vision_cache` and `_regex_cache`, meaning that toggling identity Blacklist/Whitelist filters or modifying text patterns never flushes the heavy ML models. Instead, these actions completely bypass the worker threads and loading animations, transforming into purely synchronous main-thread array filter operations that update the UI instantly (<1ms) with zero flicker or CPU overhead.
...
- [x] **Persistent Diagnostics & Logging**: Automatically initializes Python `faulthandler` to log native C/C++ segmentation faults, paired with Python's standard `logging` framework (`RotatingFileHandler` and `StreamHandler`) to write diagnostic outputs to both the console and a persistent `safemarc.log` file in the XDG app data directory. Users can dynamically configure the **Max Log File Size** and **Max Backups** from the General Settings tab. Setting the max size to `0` dynamically purges the rotating file handlers and deletes existing on-disk logs in real-time. The logging framework is fully resilient to headless and packaged `noconsole` environments (such as PyInstaller builds).
